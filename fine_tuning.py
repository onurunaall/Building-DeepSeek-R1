...
def run_ft_training() -> None:
    """
    Run the fine-tuning training loop with our curated dataset.
    This function loads the dataset, sets up our model and tokenizer, and starts training.
    """

    ft_dataset = load_dataset("bespokelabs/Bespoke-Stratos-17k", "default", split="train")
    
    ft_tokenizer = AutoTokenizer.from_pretrained(MODEL_REF, trust_remote_code=True, padding_side="right")
    ...
    ft_model = AutoModelForCausalLM.from_pretrained(MODEL_REF, trust_remote_code=True, torch_dtype=torch.bfloat16)
